...
(function () {
  ...
  function initviewCountStats(){
      ...
      var result = Math.ceil(( hoy.getDay() + 1 + numberOfDays) / 7);
      let fechaLOG = hoy.getFullYear()  + '-' + ( result );
      firebase.database().ref("OperatorLOG/"+userinflocal.uid+"/"+fechaLOG).once('value', (snapshotP) => {
          if (snapshotP.exists() && (parseInt(snapshotP.val().Points) >= 0) ) { 
            var pont = parseInt(snapshotP.val().Points); 
            for(var i = 0; i < statsvalue.length; i++){
              document.getElementById("MessageidN").innerHTML = " ( "+snapshotP.val().Points+" )";
              if(pont>=statsvalue[i].points){
                  
                  document.getElementById(i+"td").style.background = "#0dcaf0"            
                  document.getElementById("totalmoneyconvert").innerHTML = "€ :  "+(pont * statsvalue[i].valur).toFixed(2)
                  i = 10000; 
              }
            }           
          }else{
            ...
            for(var i = 0; i < statsvalue.length; i++){
              if(pontEx>=statsvalue[i].points){
                  document.getElementById(i+"td").style.background = "#0dcaf0"
                  document.getElementById("totalmoneyconvert").innerHTML = "€ :  "+(pontEx * statsvalue[i].valur).toFixed(2)
                  i = 10000; 
              }
            }
          }
      ...
      firebase.database().ref("OperatorLOG/"+userinflocal.uid).once('value', (snapshotLOGP) => {
        if (snapshotLOGP.exists()) { 
    
          snapshotLOGP.forEach((snapshotLOG) => {
            for(var i = 0; i < statsvalue.length; i++){
              if(snapshotLOG.val().Points>=statsvalue[i].points){      
                var chpoint  =      snapshotLOG.val().Pointscheck == undefined ? 0 : snapshotLOG.val().Pointscheck   
                document.getElementById("MessagesummaryWEEk").innerHTML += "<tr><td>"+userinflocal.key+"</td><td>"+userinflocal.Nickname+"</td><td>"+snapshotLOG.val().Points+"</td><td>"+chpoint+"</td><td>"+fechaLOG+"</td><td>"+((snapshotLOG.val().Points * statsvalue[i].valur)+(chpoint * 0.10)).toFixed(2)+"</td></tr>";
                  i = 10000; 
              }
            } 
  ...
  function initviewCountStatscheck(){
    let userinflocal = JSON.parse(localStorage.getItem("user"));
    let hoy = new Date();
    var oneJan = new Date(hoy.getFullYear(),0,1);
    var numberOfDays = Math.floor((hoy - oneJan) / (24 * 60 * 60 * 1000));
    ...
    firebase.database().ref("OperatorLOG/"+userinflocal.uid+"/"+fechaLOG+"/Pointscheck").once('value', (snapshotP) => {
        if (snapshotP.exists() && (parseInt(snapshotP.val().Pointscheck) >= 0) ) { 
          document.getElementById("checktablepoints").style.display = "block"
          var pont = parseInt(snapshotP.val().Pointscheck); 
          document.getElementById("CheckidN").innerHTML = " ( "+snapshotP.val().Pointscheck+" )";
          document.getElementById("8tdc").style.background = "#0dcaf0"            
          document.getElementById("totalmoneyconvertCheck").innerHTML = "€ :  "+(pont * 0.10).toFixed(2)        
        }else{
